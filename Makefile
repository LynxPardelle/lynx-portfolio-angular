# =============================================================================
# Makefile for Lynx Portfolio Angular Application
# =============================================================================
...
help: ## Show this help message with all available commands
	@echo "$(CYAN)🐳 Lynx Portfolio Angular - Docker Management$(NC)"
	@echo "$(CYAN)============================================$(NC)"
	@echo ""
...
dev: ## Start development server with hot-reload
	...
	docker-compose -p $(PROJECT_NAME) --profile dev up --build

dev-detached: ## Start development server in background
	@echo "$(CYAN)🚀 Starting development server (background)...$(NC)"
	docker-compose -p $(PROJECT_NAME) --profile dev up --build -d
...
prod: ## Start production server with SSR
	...
	docker-compose -p $(PROJECT_NAME) --profile prod up --build
...
prod-detached: ## Start production server with SSR in background
	...
	docker-compose -p $(PROJECT_NAME) --profile prod up --build -d
...
prod-no-ssr: ## Start production server without SSR (Nginx)
	...
	docker-compose -p $(PROJECT_NAME) --profile prod-no-ssr up --build
...
prod-no-ssr-detached: ## Start production server without SSR in background
	...
	docker-compose -p $(PROJECT_NAME) --profile prod-no-ssr up --build -d
...
ifndef pkg
	...
	docker-compose -p $(PROJECT_NAME) exec dev npm install $(pkg)
...
debug: ## Debug compilation errors
	...
	docker-compose -p $(PROJECT_NAME) exec dev ng build --configuration development || \
...
test: ## Run unit tests in container
	...
	docker-compose -p $(PROJECT_NAME) exec dev ng test --watch=false --browsers=ChromeHeadless

lint: ## Run linting checks
	@echo "$(CYAN)🔍 Running linting checks...$(NC)"
	docker-compose -p $(PROJECT_NAME) exec dev ng lint

build-check: ## Check if build completes successfully
	@echo "$(CYAN)🏗️ Testing production build...$(NC)"
	docker-compose -p $(PROJECT_NAME) exec dev ng build --configuration production
